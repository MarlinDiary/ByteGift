...
export default function CustomPathPage({ params }: { params: Promise<{ customPath: string }> }) {
    const { customPath } = use(params);
    ...
    useEffect(() => {
        const fetchShare = async () => {
            try {
                const response = await fetch(`http://localhost:5001/api/share/${customPath}`);
                if (!response.ok) {
